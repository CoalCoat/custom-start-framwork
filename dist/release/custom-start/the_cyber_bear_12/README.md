...
自定义开局 Perk 配置包，适用于 **Probably Stolen Playtest** 的 **一贫如洗（Rock Bottom）** 职业。
...
1. 安装 **Custom Start Framework**（将 `CustomStartFramework-1.0.0.dll` 重命名为 `CustomStartFramework.dll` 放入游戏 `Mods/`）
...
1. Install **Custom Start Framework** (copy `CustomStartFramework-1.0.0.dll` → rename to `CustomStartFramework.dll` → place in game `Mods/`)
